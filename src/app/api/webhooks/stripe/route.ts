...
import { sendTicketPurchaseConfirmationEmail, sendWorkshopRegistrationConfirmationEmail, sendAcademyEnrollmentConfirmationEmail } from "@/lib/email";
    ...
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        if (session.metadata?.type === "academy_enrollment") {
          await handleAcademyEnrollment(session);
          break;
        }
...
async function handleAcademyEnrollment(session: Stripe.Checkout.Session) {
  const applicationId = session.metadata?.application_id;
