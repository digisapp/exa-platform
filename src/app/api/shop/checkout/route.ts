...
export async function POST(request: Request) {
  try {
    ...
    // Find cart (service role — shop_carts is service-role-only under RLS;
    // ownership is verified below via user_id / session_id)
    ...
    // Create order in database (service role — checkout supports guest sessions,
    // and shop_orders/shop_order_items have no user-context write policies).
    // All inputs are validated server-side above.
    // as any needed: order_number is auto-generated by DB but typed as required
