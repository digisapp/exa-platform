...
export default function AdminTravelPage() {
  ...
  const [activeTab, setActiveTab] = useState<"trips" | "partners" | "tourism" | "applicants">("trips");

  const tabs = [
    { id: "trips" as const,      label: "Trips",      icon: Plane },
    { id: "partners" as const,   label: "Partners",   icon: Building2 },
    { id: "tourism" as const,    label: "Tourism",    icon: Globe },
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">Manage trips, partners, tourism boards, and applications</p>
        </div>
      </div>
      ...
      {activeTab === "tourism"    && <TourismTab supabase={supabase} />}
...
function TourismTab({ supabase }: { supabase: any }) {
  ...
  const [showSendDialog, setShowSendDialog] = useState(false);
  const [sendSubject, setSendSubject] = useState(DEFAULT_TOURISM_SUBJECT);
  const [sendBody, setSendBody] = useState(DEFAULT_TOURISM_BODY);
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [sending, setSending] = useState(false);
  const [sendProgress, setSendProgress] = useState<{ sent: number; failed: number; total: number } | null>(null);

  const [form, setForm] = useState({
    brand_name: "", contact_name: "", email: "", email_type: "general",
    phone: "", website_url: "", location_city: "", location_country: "", notes: "",
  });
  ...
  async function addContact() {
    if (!form.brand_name || !form.email) { toast.error("Name and email required"); return; }
    setSaving(true);
    const { error } = await (supabase.from("brand_outreach_contacts") as any).insert({
    ...
    if (error) toast.error("Failed to add contact");
      ...
      toast.success("Tourism board added");
      setShowForm(false);
      setForm({ brand_name: "", contact_name: "", email: "", email_type: "general", phone: "", website_url: "", location_city: "", location_country: "", notes: "" });
      loadContacts();
    }
    setSaving(false);
  }
  ...
  function toggleAll(list: TourismContact[]) {
    if (list.every((c) => selectedIds.has(c.id))) {
      setSelectedIds(new Set());
    } else {
  ...
  async function sendPitch() {
    const selected = contacts
      ...
      .map((c) => ({ id: c.id, email: c.email, brand_name: c.brand_name, contact_name: c.contact_name }));
    ...
    try {
      const res = await fetch("/api/admin/travel/send-outreach", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ contacts: selected, subject: sendSubject, body: sendBody }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Send failed");
      setSendProgress({ sent: data.sent, failed: data.failed, total: data.total });
      toast.success(`Sent ${data.sent} / ${data.total} emails`);
      if (data.failed === 0) {
        setTimeout(() => { setShowSendDialog(false); loadContacts(); }, 1800);
      } else {
        loadContacts();
      }
    } catch (err) {
  ...
  const filtered = contacts.filter((c) => {
    ...
    const matchSearch = !search || c.brand_name.toLowerCase().includes(search.toLowerCase())
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-3 flex-wrap">
        {[
          ...
          >
            {label} {key === "all" ? contacts.length : (statusCounts[key] || 0)}
          </button>
        ))}
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-xs">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
          <Input className="pl-9" placeholder="Search tourism boards..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        <Button onClick={openSendDialog} size="sm" variant="outline" className="border-teal-500/40 text-teal-400 hover:bg-teal-500/10">
          <Mail className="h-4 w-4 mr-1.5" /> Send Pitch
        </Button>
        <Button onClick={() => setShowForm(true)} size="sm" className="bg-teal-600 hover:bg-teal-700">
          <Plus className="h-4 w-4 mr-1.5" /> Add Board
        </Button>
      </div>
              ...
              <CardContent className="p-4 flex items-center gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <Badge className={partnerStatusColors[contact.status] || "bg-zinc-500/20 text-zinc-400"}>
                      {contact.status}
                    </Badge>
                    {contact.location_country && (
                      <span className="text-xs text-muted-foreground">
                        {contact.location_city && `${contact.location_city}, `}{contact.location_country}
                      </span>
                    )}
                  </div>
                  <div className="flex flex-wrap gap-3 mt-1 text-xs text-muted-foreground">
                  ...
                  {contact.notes && (
                    <p className="text-xs text-muted-foreground mt-1 italic truncate">&ldquo;{contact.notes}&rdquo;</p>
                  )}
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <Select
                  ...
                  >
                    <SelectTrigger className="h-7 w-32 text-xs">
                      {updatingId === contact.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <SelectValue />}
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="new">New</SelectItem>
                      <SelectItem value="contacted">Contacted</SelectItem>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            <div>
              <div className="flex items-center justify-between mb-2">
                <Label>Recipients ({selectedIds.size} selected)</Label>
                <button onClick={() => toggleAll(contacts)} className="text-xs text-teal-400 hover:text-teal-300">
                  {contacts.every((c) => selectedIds.has(c.id)) ? "Deselect All" : "Select All"}
                </button>
              </div>
              <div className="max-h-36 overflow-y-auto space-y-1 border border-zinc-800 rounded-lg p-2 bg-zinc-950">
                {contacts.map((c) => (
                  <label key={c.id} className="flex items-center gap-2 px-2 py-1 rounded hover:bg-zinc-900 cursor-pointer">
                    <input
                      type="checkbox"
                    ...
                    />
                    <span className="text-sm font-medium flex-1">{c.brand_name}</span>
                    <span className="text-xs text-zinc-500 truncate max-w-[180px]">{c.email}</span>
                  </label>
                ))}
              </div>
            </div>
            <div>
              <Label>Subject</Label>
              <Input value={sendSubject} onChange={(e) => setSendSubject(e.target.value)} className="mt-1" />
            </div>
            <div>
              <Label>
                Email Body{" "}
              ...
              <Textarea value={sendBody} onChange={(e) => setSendBody(e.target.value)} rows={14} className="mt-1 font-mono text-xs" />
            </div>
            {sendProgress && (
              <div className="flex items-center gap-4 px-4 py-3 rounded-lg bg-zinc-900 border border-zinc-800">
                <div className="text-center">
            ...
            >
              {sending ? (
                ...
                <><Mail className="h-4 w-4 mr-2" /> Send to {selectedIds.size} Board{selectedIds.size !== 1 ? "s" : ""}</>
              )}
            </Button>
          </div>
        </DialogContent>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Add Tourism Board</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <Label>Board / Organization Name *</Label>
              <Input value={form.brand_name} onChange={(e) => setForm({ ...form, brand_name: e.target.value })}
                placeholder="Visit Dubai" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>Contact Name</Label>
              ...
              <div>
                <Label>Email *</Label>
                <Input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}
                  placeholder="press@visitdubai.com" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
              ...
              <div>
                ...
                <Select value={form.email_type} onValueChange={(v) => setForm({ ...form, email_type: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
            ...
            <div>
              <Label>Notes</Label>
              <Textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })}
                rows={2} placeholder="Active influencer programs, key contacts..." />
            </div>
            <Button onClick={addContact} disabled={saving} className="w-full bg-teal-600 hover:bg-teal-700">
              {saving ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null}
              Add Tourism Board
            </Button>
          </div>
