...
/**
 * Map URL-friendly day param (e.g. "may-25") to the DAY_META key ("May 25").
 */
function parseDayParam(param: string): string | null {
  const match = param.match(/^may-?(\d{1,2})$/i);
  if (!match) return null;
  const key = `May ${match[1]}`;
...
function classifyTier(name: string): {
  ...
  if (lower.includes("bottle"))
    return { type: "bottle", label: "VIP Bottle Table" };
  if (lower.includes("first row"))
    return { type: "first", label: "First Row" };
  if (lower.includes("second row"))
    return { type: "second", label: "Second Row" };
  if (lower.includes("third row"))
    return { type: "third", label: "Third Row" };
...
export function TicketPageContent({
  ...
  // Pre-select day from URL param on mount
  useEffect(() => {
    if (initialDay) {
      const resolved = parseDayParam(initialDay);
      if (resolved && tiersByDay[resolved]?.length) {
        setSelectedDay(resolved);
      }
    }
  }, [initialDay, tiersByDay]);
