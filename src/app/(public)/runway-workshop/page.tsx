...
export async function generateMetadata(): Promise<Metadata> {
  ...
  if (!data) {
    return {
      title: "Runway Workshop | EXA Models",
      description: "Learn the art of the runway walk with EXA Models.",
    };
  }
  ...
  return {
    ...
    description,
    alternates: {
      canonical: "https://www.examodels.com/workshops/runway-workshop",
    },
    openGraph: {
      title,
      description,
      url: "https://www.examodels.com/workshops/runway-workshop",
      type: "website",
      siteName: "EXA Models",
...
export default async function RunwayWorkshopPage() {
  const supabase = await createClient();

  // Get workshop by hardcoded slug
          ...
          <main className="container px-4 md:px-8 py-8">
            ...
            <div className="max-w-2xl mx-auto text-center py-24">
              ...
              <p className="text-muted-foreground">
                Details will be announced shortly. Check back soon!
              </p>
            </div>
          </main>
        </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-4 mt-4 text-sm">
                  <div className="flex items-center gap-2 text-muted-foreground">
                    ...
                    <span>
                      {format(workshopDate, "EEEE, MMMM d, yyyy")}
                    </span>
                  </div>
                  {workshop.start_time && (
                    <div className="flex items-center gap-2 text-muted-foreground">
                      ...
                      <span>
                        {format(
                          new Date(`2000-01-01T${workshop.start_time}`),
                          "h:mm a"
                        )}
                        {workshop.end_time &&
                          ` - ${format(new Date(`2000-01-01T${workshop.end_time}`), "h:mm a")}`}
                      </span>
                    </div>
                  )}
                  ...
                  <CardContent className="pt-6">
                    ...
                    <div className="prose prose-sm prose-invert max-w-none">
                      {workshop.description.split("\n").map((paragraph, i) => (
                        <p
                          key={i}
                          className="text-muted-foreground mb-3 last:mb-0"
                        >
