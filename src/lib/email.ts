      ...
      <td align="center">
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #1a1a1a; border-radius: 16px; overflow: hidden;">
          ...
          <tr>
            <td style="padding: 40px 30px;">
              <p style="margin: 0 0 20px; color: #ffffff; font-size: 18px;">
                Great news, ${escapeHtml(clientName)}!
              </p>
              <p style="margin: 0 0 30px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
                <strong style="color: #ffffff;">${escapeHtml(modelName)}</strong> has accepted your booking request! Your coins have been secured in escrow.
              </p>
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0">
                      ...
                      <tr>
                        <td style="padding: 15px 0; border-bottom: 1px solid #404040;">
                          ...
                          <p style="margin: 0; color: #ffffff; font-size: 16px; font-weight: 500;">${escapeHtml(modelName)}</p>
                        </td>
                      </tr>
                      <tr>
                        <td style="padding: 15px 0; border-bottom: 1px solid #404040;">
                          <p style="margin: 0 0 5px; color: #71717a; font-size: 12px; text-transform: uppercase;">Service</p>
                          <p style="margin: 0; color: #ffffff; font-size: 16px;">${escapeHtml(serviceType)}</p>
                        </td>
                      </tr>
                      <tr>
                        <td style="padding: 15px 0; border-bottom: 1px solid #404040;">
                ...
                <tr>
                  <td style="padding: 15px; background-color: #052e16; border-radius: 8px; border-left: 3px solid #10b981;">
                    ...
                    <p style="margin: 0; color: #86efac; font-size: 14px;">
                      Coordinate the details with ${escapeHtml(modelName)} and confirm the booking when ready. Coins will be released to the model upon completion.
                    </p>
                  </td>
                </tr>
              </table>
                ...
                <tr>
                  <td align="center">
                    <a href="${modelProfileUrl}" style="display: inline-block; color: #a1a1aa; text-decoration: none; font-size: 14px;">
                      View ${escapeHtml(modelName)}'s Profile
                    </a>
                  </td>
                </tr>
              </table>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 30px;">
                <tr>
                  <td style="padding: 15px; background-color: #262626; border-radius: 8px; border-left: 3px solid #71717a;">
                    <p style="margin: 0 0 5px; color: #71717a; font-size: 12px;">Message from ${escapeHtml(modelName)}:</p>
                    <p style="margin: 0; color: #ffffff; font-size: 14px; font-style: italic;">"${escapeHtml(reason)}"</p>
                  </td>
                </tr>
              </table>`
      : "";
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #1a1a1a; border-radius: 16px; overflow: hidden;">
          ...
          <tr>
            <td style="padding: 40px 30px;">
              ...
              <p style="margin: 0 0 30px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
                Unfortunately, <strong style="color: #ffffff;">${escapeHtml(modelName)}</strong> wasn't able to accept your booking request. Don't worry - no coins were charged.
              </p>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #1a1a1a; border-radius: 16px; overflow: hidden;">
          ...
          <tr>
            <td style="padding: 40px 30px;">
              ...
              <p style="margin: 0 0 30px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
                <strong style="color: #ffffff;">${escapeHtml(callerName)}</strong> is trying to ${callTypeLabel} call you! Open your chats to accept the call.
              </p>
...
// Missed-call recovery: a fan rang and the model never picked up. ONE email
// per model per 6h window (deduped via chat_nudges_sent 'missed_call' in the
// sweep-stale-calls notify pass) — positive re-engagement, never a scold.
// callerName is the fan's @username/display_name (fans have no real names on
// file); the model is greeted by their own first name (own-inbox exception).
export async function sendMissedCallEmail({
  ...
  callType?: "video" | "voice";
  callRate: number;
  missedAt?: string | Date | null;
}) {
  try {
    if (await isEmailUnsubscribed(to, "notification")) {
      logger.info("Skipping missed call email - recipient is unsubscribed", { to });
      return;
    }
    const resend = getResendClient();
    const dashboardUrl = `${BASE_URL}/dashboard`;
    const callTypeLabel = callType === "voice" ? "voice" : "video";
    const missedAtLabel = missedAt
      ? new Date(missedAt).toLocaleString("en-US", {
          month: "short",
          day: "numeric",
          hour: "numeric",
          minute: "2-digit",
    ...
    const { data, error } = await resend.emails.send({
      ...
      replyTo: REPLY_TO_EMAIL,
      to: [to],
      subject: `💜 ${callerName} tried to ${callTypeLabel} call you`,
      html: `
<!DOCTYPE html>
<html>
<head>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #1a1a1a; border-radius: 16px; overflow: hidden;">
          ...
          <tr>
            <td style="padding: 40px 30px;">
              ...
              <p style="margin: 0 0 30px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
                <strong style="color: #ffffff;">${escapeHtml(callerName)}</strong> tried to ${callTypeLabel} call you${missedAtLabel ? ` at <strong style="color: #ffffff;">${escapeHtml(missedAtLabel)}</strong>` : " just now"}, but the call rang out. Fans who call once almost always try again — be ready for the next one.
              </p>
                ...
                <tr>
                  <td style="padding: 15px; background-color: #1e1b4b; border-radius: 8px; border-left: 3px solid #8b5cf6;">
                    <p style="margin: 0; color: #c4b5fd; font-size: 14px; line-height: 1.6;">
                      💡 Flip on <strong style="color: #ffffff;">Available for calls</strong> on your dashboard and fans can ring you even when you're not on EXA — we'll ping you the moment a call comes in.
                    </p>
                  </td>
                </tr>
              </table>
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background-color: #18181b; border-radius: 16px; overflow: hidden; border: 1px solid #27272a;">
          ...
          <tr>
            <td style="padding: 40px 30px;">
              ...
              <p style="margin: 0 0 30px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
                <strong style="color: #ffffff;">${escapeHtml(brandName)}</strong> has sent you an exciting offer!
              </p>
                  ...
                  <td style="padding: 25px;">
                    ...
                    <table cellpadding="0" cellspacing="0" style="margin-bottom: 10px;">
                      <tr>
                        ...
                        <td style="color: #ffffff; font-size: 14px;">${escapeHtml(eventDate)}${eventTime ? ` at ${escapeHtml(eventTime)}` : ""}</td>
                      </tr>
                    </table>
                    ` : ""}
                    ...
                    <table cellpadding="0" cellspacing="0">
                      <tr>
                        <td style="color: #71717a; font-size: 14px; padding-right: 10px;">💰</td>
                        <td style="color: #10b981; font-size: 14px; font-weight: 600;">${escapeHtml(compensation)}</td>
                      </tr>
                    </table>
                    ` : ""}
                  </td>
                ...
                <tr>
                  <td style="padding: 25px;">
                    ...
                    <table cellpadding="0" cellspacing="0" style="margin-bottom: 10px;">
                      <tr>
                        <td style="color: #71717a; font-size: 14px; padding-right: 10px;">📅</td>
                        <td style="color: #ffffff; font-size: 14px;">${escapeHtml(gigDate)}</td>
                      </tr>
                    </table>
                    ` : ""}
                    ...
                    <table cellpadding="0" cellspacing="0">
                      <tr>
                        <td style="color: #71717a; font-size: 14px; padding-right: 10px;">📍</td>
                        <td style="color: #ffffff; font-size: 14px;">${escapeHtml(gigLocation)}</td>
                      </tr>
                    </table>
                    ` : ""}
                  </td>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #1a1a1a; border-radius: 16px; overflow: hidden;">
          ...
          <tr>
            <td style="padding: 40px 30px;">
              ...
              <p style="margin: 0 0 20px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
                Thank you for your interest in <strong style="color: #ffffff;">${escapeHtml(gigTitle)}</strong>. Unfortunately, we weren't able to accept your application at this time.
              </p>
              <p style="margin: 0 0 30px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background-color: #18181b; border-radius: 16px; overflow: hidden; border: 1px solid #27272a;">
          ...
          <tr>
            <td style="padding: 40px 30px;">
              ...
              <p style="margin: 0 0 30px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
                Just a friendly reminder that your confirmed event with <strong style="color: #ffffff;">${escapeHtml(brandName)}</strong> is coming up soon!
              </p>
                ...
                <tr>
                  <td style="padding: 25px;">
                    ...
                    <table cellpadding="0" cellspacing="0" style="margin-bottom: 10px;">
                      <tr>
                        <td style="color: #71717a; font-size: 14px; padding-right: 10px;">📅</td>
                        <td style="color: #fbbf24; font-size: 16px; font-weight: 600;">${escapeHtml(eventDate)}${eventTime ? ` at ${escapeHtml(eventTime)}` : ""}</td>
                      </tr>
                    </table>
                    ...
                    <table cellpadding="0" cellspacing="0">
                      <tr>
                        <td style="color: #71717a; font-size: 14px; padding-right: 10px;">📞</td>
                        <td style="color: #ffffff; font-size: 14px;">${escapeHtml(brandContact)}</td>
                      </tr>
                    </table>
                    ` : ""}
                  </td>
...
export async function sendMiamiSwimWeekInviteEmail({
  ...
  try {
    ...
    const greeting = name ? `Hey ${escapeHtml(name)}!` : "Hey Beautiful!";
...
export async function sendMswUrgentProfilePhotoEmail({
  to,
  modelName,
}: {
  to: string;
  ...
  try {
    ...
    const { data, error } = await resend.emails.send({
      ...
      replyTo: REPLY_TO_EMAIL,
      to: [to],
      subject: "URGENT: Upload Your Profile Photo to Be Considered for Miami Swim Week",
      html: `
<!DOCTYPE html>
<html>
<head>
    ...
    <tr>
      <td align="center">
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #1a1a1a; border-radius: 16px; overflow: hidden;">
          ...
          <tr>
            <td style="background: linear-gradient(135deg, #ec4899 0%, #8b5cf6 100%); padding: 40px 30px; text-align: center;">
              <p style="margin: 0 0 8px; color: rgba(255,255,255,0.95); font-size: 12px; letter-spacing: 3px; text-transform: uppercase; font-weight: 700;">⚠ Urgent &middot; Action Required</p>
              <h1 style="margin: 0; color: white; font-size: 28px; font-weight: bold;">
                ${greeting}
              </h1>
              <p style="margin: 10px 0 0; color: rgba(255,255,255,0.95); font-size: 16px;">
                Upload your profile photo for Miami Swim Week
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding: 40px 30px;">
              <p style="margin: 0 0 20px; color: #ffffff; font-size: 18px; line-height: 1.5;">
                You're approved on EXA — but your profile is missing a photo.
              </p>
              <p style="margin: 0 0 25px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
                Designers cannot consider you for <strong style="color:#ffffff;">Miami Swim Week</strong> until a clear profile photo is on your EXA model page. Upload one now to stay in the running.
              </p>

              <!-- CTA -->
              <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 24px; background-color: #111111; border: 1px solid #262626; border-radius: 12px;">
                <tr>
                  <td style="padding: 24px; text-align: center;">
                    <p style="margin: 0 0 6px; color: #ec4899; font-size: 12px; font-weight: 700; letter-spacing: 1px; text-transform: uppercase;">Step 1 &middot; Required</p>
                    <p style="margin: 0 0 18px; color: #ffffff; font-size: 16px; font-weight: 600;">
                      Upload your profile photo on your EXA model page
                    </p>
                    <a href="${settingsUrl}" style="display: inline-block; background: linear-gradient(135deg, #ec4899 0%, #8b5cf6 100%); color: white; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-weight: 700; font-size: 15px;">
              ...
              <p style="margin: 0; color: #fbbf24; font-size: 14px; line-height: 1.6; text-align: center; font-weight: 600;">
                Spots are filling fast. Upload your photo today or you will not be considered.
              </p>
            </td>
          </tr>
    ...
    <tr>
      <td align="center">
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #1a1a1a; border-radius: 16px; overflow: hidden;">
          ...
          <tr>
            <td style="padding: 0;">
              <img src="${coverImageUrl}" alt="${escapeHtml(gigTitle)}" style="width: 100%; height: auto; display: block;" />
            </td>
          </tr>
          ` : ""}
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 30px; background-color: #262626; border-radius: 12px; overflow: hidden;">
                <tr>
                  <td style="padding: 25px;">
                    ...
                    <h2 style="margin: 0 0 15px; color: #ffffff; font-size: 22px;">${escapeHtml(gigTitle)}</h2>
...
export async function sendMswAvailabilityUrgentSelectEmail({
  to,
  modelName,
}: {
  to: string;
  ...
  try {
    ...
    const { data, error } = await resend.emails.send({
      ...
      replyTo: REPLY_TO_EMAIL,
      to: [to],
      subject: "URGENT: Select Days Availability — Miami Swim Week",
      html: `
<!DOCTYPE html>
<html>
<head>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #111111; border-radius: 16px; overflow: hidden; border: 1px solid rgba(239, 68, 68, 0.25);">
          ...
          <tr>
            <td style="padding: 40px 30px;">
              ...
              <p style="margin: 0 0 22px; color: #a1a1aa; font-size: 16px; line-height: 1.65;">
                You're <strong style="color: #ffffff;">confirmed for Miami Swim Week</strong> — but we still need you to tell us <strong style="color: #ffffff;">which days you're available to walk</strong>. Without your availability, designers can't cast you.
              </p>
              <p style="margin: 0 0 30px; color: #a1a1aa; font-size: 16px; line-height: 1.65;">
                Designers are building their casts <strong style="color: #ffffff;">right now</strong>. Please sign in and select your days as soon as possible — every hour matters.
              </p>
                ...
                <tr>
                  <td style="padding: 26px;">
                    ...
                    <table cellpadding="0" cellspacing="0" width="100%">
                      ...
                      <tr>
                        <td style="padding: 0 0 12px; color: #ffffff; font-size: 15px; line-height: 1.5;">
                          <strong style="color: #ef4444;">3.</strong> &nbsp;Tap each day you're available on the week calendar
                        </td>
                      </tr>
                      <tr>
                        <td style="padding: 0; color: #ffffff; font-size: 15px; line-height: 1.5;">
              ...
              <p style="margin: 28px 0 0; color: #71717a; font-size: 13px; line-height: 1.55; text-align: center;">
                Pick all the days that work — even one is fine. Designers are casting now.
              </p>
...
export async function sendMswAvailabilityReminderEmail({
  to,
  modelName,
}: {
  to: string;
  ...
  try {
    ...
    const { data, error } = await resend.emails.send({
      ...
      replyTo: REPLY_TO_EMAIL,
      to: [to],
      subject: "Reminder: Update your Miami Swim Week availability",
      html: `
<!DOCTYPE html>
<html>
<head>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #111111; border-radius: 16px; overflow: hidden; border: 1px solid rgba(236, 72, 153, 0.25);">
          ...
          <tr>
            <td style="padding: 40px 30px;">
              ...
              <p style="margin: 0 0 22px; color: #a1a1aa; font-size: 16px; line-height: 1.65;">
                You're <strong style="color: #ffffff;">confirmed for Miami Swim Week</strong>. Please <strong style="color: #ffffff;">make sure your availability is up to date</strong> so designers can cast you for the right days.
              </p>
              <p style="margin: 0 0 30px; color: #a1a1aa; font-size: 16px; line-height: 1.65;">
                Sign in to your EXA profile, open the <em style="color: #f9a8d4; font-style: normal;">Miami Swim Week schedule</em>, and select every day you're available to walk.
              </p>
                ...
                <tr>
                  <td style="padding: 26px;">
                    ...
                    <table cellpadding="0" cellspacing="0" width="100%">
                      ...
                      <tr>
                        <td style="padding: 0 0 12px; color: #ffffff; font-size: 15px; line-height: 1.5;">
                          <strong style="color: #ec4899;">2.</strong> &nbsp;You'll see the <em style="color: #f9a8d4; font-style: normal;">Miami Swim Week schedule</em> on your dashboard
                        </td>
                      </tr>
                      <tr>
                        <td style="padding: 0 0 12px; color: #ffffff; font-size: 15px; line-height: 1.5;">
                          <strong style="color: #ec4899;">3.</strong> &nbsp;Select every day you're available to walk
                        </td>
                      </tr>
                      <tr>
                        <td style="padding: 0; color: #ffffff; font-size: 15px; line-height: 1.5;">
                          <strong style="color: #ec4899;">4.</strong> &nbsp;Hit <strong>Save Availability</strong>
                        </td>
                      </tr>
                    </table>
                  </td>
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center">
                    <a href="${dashboardUrl}" style="display: inline-block; background: linear-gradient(135deg, #ec4899 0%, #8b5cf6 60%, #06b6d4 100%); color: white; text-decoration: none; padding: 16px 44px; border-radius: 10px; font-weight: 700; font-size: 15px; letter-spacing: 0.3px; box-shadow: 0 0 24px rgba(236, 72, 153, 0.35);">
              ...
              <p style="margin: 28px 0 0; color: #71717a; font-size: 13px; line-height: 1.55; text-align: center;">
                Already set your days? Open the schedule and double-check — designers are casting now.
              </p>
      ...
      <td align="center">
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #1a1a1a; border-radius: 16px; overflow: hidden;">
          ...
          <tr>
            <td style="padding: 40px 30px;">
              <p style="margin: 0 0 20px; color: #ffffff; font-size: 18px;">
                Hey ${escapeHtml(contactName)},
              </p>
              <p style="margin: 0 0 20px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
                EXA Models is launching an exclusive content program for swimwear brands like ${escapeHtml(brandName)}.
              </p>
              <p style="margin: 0 0 20px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #1a1a1a; border-radius: 16px; overflow: hidden;">
          ...
          <tr>
            <td style="padding: 40px 30px;">
              ...
              <p style="margin: 0 0 20px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
                Thank you for applying to the EXA Models Swimwear Content Program with <strong style="color: #ffffff;">${escapeHtml(brandName)}</strong>.
              </p>
              <p style="margin: 0 0 20px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #1a1a1a; border-radius: 16px; overflow: hidden;">
          ...
          <tr>
            <td style="padding: 40px 30px;">
              ...
              <p style="margin: 0 0 20px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
                Great news! <strong style="color: #ffffff;">${escapeHtml(brandName)}</strong> has been approved for the EXA Models Swimwear Content Program.
              </p>
              <p style="margin: 0 0 20px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #1a1a1a; border-radius: 16px; overflow: hidden;">
          ...
          <tr>
            <td style="padding: 40px 30px;">
              ...
              <p style="margin: 0 0 20px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
                Your Month ${paymentMonth} payment for <strong style="color: #ffffff;">${escapeHtml(brandName)}</strong> is due.
              </p>
...
export async function sendBrandOutreachEmail({
  ...
  try {
    ...
    const resolvedCtaUrl = ctaUrl || `${BASE_URL}/swimweek`;
    const resolvedCtaText = ctaText || "View Miami Swim Week 2026";
    const resolvedFrom = fromEmail || "partnerships@examodels.com";
    const resolvedFromName = fromName || "EXA Models Partnerships";

    // Replace template variables (escape for HTML safety)
    ...
    const htmlBody = personalizedBody
      .split("\n")
      .map((line) => (line.trim() === "" ? "" : `<p style="margin: 0 0 14px; color: #d4d4d4; font-size: 15px; line-height: 1.7;">${escapeHtml(line)}</p>`))
      .join("\n");
          ...
          <tr>
            <td style="background-color: #111111; border-radius: 0 0 16px 16px; padding: 40px 36px;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 36px;">
                <tr>
                  <td align="center">
                    <a href="${resolvedCtaUrl}" style="display: inline-block; background: linear-gradient(135deg, #ec4899 0%, #8b5cf6 100%); color: white; text-decoration: none; padding: 16px 44px; border-radius: 10px; font-weight: 700; font-size: 16px; letter-spacing: 0.3px;">
                      ${resolvedCtaText}
                    </a>
                  </td>
                </tr>
          ...
          <tr>
            <td style="padding: 0 30px 40px;">
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center">
                    <a href="${resolvedCtaUrl}" style="display: inline-block; background: linear-gradient(135deg, #ec4899 0%, #8b5cf6 100%); color: white; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-weight: 600; font-size: 16px;">
                      ${resolvedCtaText}
                    </a>
                  </td>
                </tr>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px;">
          ...
          <tr>
            <td style="background-color: #0d1117; border-radius: 0 0 16px 16px; padding: 40px 36px;">
              ...
              <p style="margin: 0 0 32px; color: #cbd5e1; font-size: 15px; line-height: 1.75;">We bring our team of 7 people (5 influencer models + photographer and director) and spend four days capturing the full experience — the property, the culture, and the destination itself — in a cinematic, elevated way.</p>

              <!-- What You Receive -->
              <p style="margin: 0 0 12px; color: #ffffff; font-size: 13px; font-weight: 700; letter-spacing: 1px; text-transform: uppercase;">What You Receive</p>
              <p style="margin: 0 0 8px; color: #cbd5e1; font-size: 15px; line-height: 1.6;"><span style="color: #3b82f6; margin-right: 8px;">◆</span><strong style="color: #ffffff;">Instagram posts &amp; stories</strong> from each of our 5 models</p>
              <p style="margin: 0 0 8px; color: #cbd5e1; font-size: 15px; line-height: 1.6;"><span style="color: #3b82f6; margin-right: 8px;">◆</span><strong style="color: #ffffff;">10+ short videos</strong> posted across Reels &amp; TikTok</p>
              <p style="margin: 0 0 8px; color: #cbd5e1; font-size: 15px; line-height: 1.6;"><span style="color: #3b82f6; margin-right: 8px;">◆</span><strong style="color: #ffffff;">One full cinematic video</strong> (30–60 sec) — usable for your ads</p>
              <p style="margin: 0 0 8px; color: #cbd5e1; font-size: 15px; line-height: 1.6;"><span style="color: #3b82f6; margin-right: 8px;">◆</span><strong style="color: #ffffff;">20 professional photos</strong> — website, ads, social</p>
              <p style="margin: 0 0 32px; color: #cbd5e1; font-size: 15px; line-height: 1.6;"><span style="color: #3b82f6; margin-right: 8px;">◆</span>Every post <strong style="color: #ffffff;">tags your property</strong></p>
      ...
      <td align="center">
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #1a1a1a; border-radius: 16px; overflow: hidden;">
          ...
          <tr>
            <td style="padding: 40px 30px;">
              <p style="margin: 0 0 20px; color: #ffffff; font-size: 18px;">
                Congratulations ${escapeHtml(winnerName)}!
              </p>
              <p style="margin: 0 0 20px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
                You won <strong style="color: #ffffff;">"${escapeHtml(auctionTitle)}"</strong> by <strong style="color: #ffffff;">${escapeHtml(modelName)}</strong> with a winning bid of <strong style="color: #ec4899;">${amount} coins</strong>.
              </p>
 ...
 */
export async function sendUnreadMessageNudgeEmail({
  to,
  recipientName,
  senderName,
  messagePreview,
  ...
  try {
    ...
    const truncatedPreview = messagePreview.length > 150 ? messagePreview.slice(0, 150) + "..." : messagePreview;
    const timeLabel = hoursAgo >= 48 ? `${Math.round(hoursAgo / 24)} days ago` : `${hoursAgo} hours ago`;

    const { data, error } = await resend.emails.send({
      from: FROM_EMAIL,
      replyTo: REPLY_TO_EMAIL,
      to: [to],
      subject: `You have an unread message from ${senderName}`,
      html: `
<!DOCTYPE html>
<html>
<head>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #1a1a1a; border-radius: 16px; overflow: hidden;">
          ...
          <tr>
            <td style="padding: 40px 30px;">
              ...
              <p style="margin: 0 0 20px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
                <strong style="color: #ffffff;">${escapeHtml(senderName)}</strong> sent you a message <strong style="color: #ec4899;">${timeLabel}</strong> and is still waiting for a reply.
              </p>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 30px;">
                <tr>
                  <td style="padding: 15px; background-color: #262626; border-radius: 8px; border-left: 3px solid #8b5cf6;">
                    <p style="margin: 0 0 5px; color: #71717a; font-size: 12px;">Their message:</p>
                    <p style="margin: 0; color: #ffffff; font-size: 14px; font-style: italic;">"${escapeHtml(truncatedPreview)}"</p>
                  </td>
                </tr>
              </table>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 20px;">
                <tr>
                  <td align="center">
                    <a href="${conversationUrl}" style="display: inline-block; padding: 14px 32px; background: linear-gradient(135deg, #8b5cf6 0%, #ec4899 100%); color: white; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px;">
                      Reply Now
                    </a>
                  </td>
                </tr>
...
export async function sendMswPendingPhotoRequiredEmail({
  to,
  modelName,
}: {
  to: string;
  ...
  try {
    ...
    const { data, error } = await resend.emails.send({
      ...
      replyTo: REPLY_TO_EMAIL,
      to: [to],
      subject: "URGENT: Add a profile photo to keep your Miami Swim Week application",
      html: `
<!DOCTYPE html>
<html>
<head>
...
<body style="margin: 0; padding: 0; background-color: #0a0a0a; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #0a0a0a; padding: 40px 20px;">
    <tr>
      <td align="center">
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #1a1a1a; border-radius: 16px; overflow: hidden;">
          ...
          <tr>
            <td style="background: linear-gradient(135deg, #ec4899 0%, #8b5cf6 100%); padding: 40px 30px; text-align: center;">
              <p style="margin: 0 0 8px; color: rgba(255,255,255,0.85); font-size: 13px; letter-spacing: 2px; text-transform: uppercase;">URGENT &middot; Miami Swim Week 2026</p>
              <h1 style="margin: 0; color: white; font-size: 28px; font-weight: bold;">
                ${greeting}
              </h1>
              <p style="margin: 10px 0 0; color: rgba(255,255,255,0.9); font-size: 16px;">
                Your application is pending — one thing missing
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding: 40px 30px;">
              <p style="margin: 0 0 20px; color: #ffffff; font-size: 18px; line-height: 1.5;">
                Thanks for applying to <strong>Miami Swim Week</strong>.
              </p>
              <p style="margin: 0 0 25px; color: #a1a1aa; font-size: 16px; line-height: 1.6;">
                We're reviewing applicants now, but your profile is missing a photo. Designers and our casting team can't consider you without one. To stay in the running, please upload a clear profile picture as soon as possible.
              </p>

              <!-- CTA -->
              <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 28px; background-color: #111111; border: 1px solid #262626; border-radius: 12px;">
                <tr>
                  <td style="padding: 24px; text-align: center;">
                    <p style="margin: 0 0 16px; color: #ffffff; font-size: 16px; font-weight: 600;">
                      Upload your profile photo
                    </p>
                    <a href="${settingsUrl}" style="display: inline-block; background: linear-gradient(135deg, #ec4899 0%, #8b5cf6 100%); color: white; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-weight: 600; font-size: 15px;">
              ...
              <p style="margin: 0; color: #71717a; font-size: 14px; line-height: 1.6; text-align: center;">
                Slots are filling fast. Get your photo up today so you don't miss the cut.
              </p>
            </td>
          </tr>
    ...
    logger.error("MSW pending photo-required email error", error);
    return { success: false, error };
  }
}
