...
export function verifyEmailToken(
  ...
  try {
    ...
    if (parts.length !== 2) {
      logger.error("Token verify: wrong number of parts", undefined, { partsLength: parts.length });
      return null;
    }
    ...
    if (expectedSignature.length !== providedSignature.length) {
      logger.error("Token verify: signature length mismatch", undefined, { expected: expectedSignature.length, got: providedSignature.length });
      return null;
    }

    if (!crypto.timingSafeEqual(expectedSignature, providedSignature)) {
      logger.error("Token verify: signature mismatch (HMAC doesn't match)");
      return null;
    }
    ...
    if (segments.length !== 3) {
      logger.error("Token verify: payload has wrong segments", undefined, { segmentsLength: segments.length });
      return null;
    }
    ...
    if (isNaN(timestamp)) {
      logger.error("Token verify: invalid timestamp");
      return null;
    }
    ...
    if (now - timestamp > expirySeconds) {
      logger.error("Token verify: expired", undefined, { age: now - timestamp, maxSeconds: expirySeconds });
      return null;
    }

    return { modelId, gigId };
  } catch (err) {
    logger.error("Token verify: exception", err);
    return null;
  }
}
