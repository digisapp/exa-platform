...
export function FlyerDesigner({ settings, onChange }: FlyerDesignerProps) {
  ...
  return (
    <div className="space-y-4 text-sm">
      ...
      {/* ── Pattern Overlays ── */}
      <Section title="Pattern Overlays">
        <div className="grid grid-cols-3 gap-2">
          {PATTERN_PRESETS.map((pattern) => (
            <button
              ...
              className="flex flex-col items-center gap-1.5 px-3 py-2.5 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 transition-all text-xs text-white/60 hover:text-white/80"
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={pattern.svg}
                alt={pattern.name}
                className="w-8 h-8 object-cover rounded bg-white/10"
              />
              {pattern.name}
            </button>
          ))}
        ...
        <p className="text-[10px] text-white/25 mt-1">
          Adds as full-canvas overlay — adjust opacity below
        </p>
      </Section>
