...
const FROM = "EXA Models <nathan@examodels.com>";
const SUBJECT = "Join Us for Miami Swim Week 2026";

// Fetch first 100 new sponsor contacts
...
<body style="margin: 0; padding: 0; background-color: #050505; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
          ...
          <tr>
            <td style="background-color: #111111; border-radius: 0 0 16px 16px; padding: 40px 36px;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="border-top: 1px solid #262626; padding-top: 24px;">
                <tr>
                  <td>
                    <p style="margin: 0 0 4px; color: #ffffff; font-weight: 600; font-size: 14px;">EXA Models</p>
  ...
  try {
    ...
    if (error) {
      console.error(`✗ ${contact.brand_name} <${contact.email}>:`, error.message);
      failed++;
    } else {
      ...
      console.log(`✓ ${contact.brand_name} <${contact.email}>`);
      sent++;
    }
  } catch (err) {
    console.error(`✗ ${contact.brand_name} <${contact.email}>:`, err);
    failed++;
  }

  // 600ms delay between sends (stay under 2/sec Resend rate limit)
