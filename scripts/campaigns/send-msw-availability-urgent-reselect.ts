/**
 * Sends the URGENT reselect email to every model confirmed (accepted gig
 * application) for any Miami Swim Week gig. Asks them to sign in and
 * re-check the days they're available because of a labeling issue on the
 * availability picker that may have saved the wrong dates.
 *
 * Usage:
 *   npx tsx scripts/campaigns/send-msw-availability-urgent-reselect.ts            # dry run
 *   npx tsx scripts/campaigns/send-msw-availability-urgent-reselect.ts --send     # actually send
 */
...
import { sendMswAvailabilityUrgentReselectEmail } from "../../src/lib/email";
...
async function main() {
  ...
  for (const m of recipients) {
    const name = m.first_name || m.username || "Model";
    try {
      const result = await sendMswAvailabilityUrgentReselectEmail({
        to: m.email!,
        modelName: name,
      });
