...
async function main() {
  ...
  const confirmedIds = Array.from(new Set((apps || []).map((a) => a.model_id)));
  console.log(`Confirmed model IDs: ${confirmedIds.length}`);

  const { data: availRows, error: availErr } = await supabase
    .from("gig_availability")
    .select("model_id")
    .in("gig_id", gigIds)
    .in("model_id", confirmedIds);

  if (availErr) {
    console.error("Failed to fetch availability rows:", availErr);
    process.exit(1);
  }

  const respondedIds = new Set((availRows || []).map((r: any) => r.model_id));
  console.log(`Already responded: ${respondedIds.size}`);

  const nonResponderIds = confirmedIds.filter((id) => !respondedIds.has(id));
  console.log(`Non-responders: ${nonResponderIds.length}`);

  if (nonResponderIds.length === 0) {
    console.log("Everyone has responded. Nothing to send.");
    return;
  }
